    ...
    .query(async ({ ctx, input }) => {
      ...
      const leaderboardQuery = ctx.db
        .select({
          ...
          correctAnswers: sql<
            number
          >`SUM(CASE WHEN ${responses.isCorrect} = true THEN 1 ELSE 0 END)`,
          totalTimeTaken: sql<number>`COALESCE(SUM(${responses.timeTaken}), 0)`,
          // Get session information for completion time calculation
          sessionStartedAt: sql<
            string
          >`MIN(${participantSessions.sessionStartedAt})`, // First session start
          lastActivityAt: sql<
            string
          >`MAX(${participantSessions.lastActivityAt})`, // Latest activity
          isCompleted: sql<
            boolean
          >`BOOL_AND(${participantSessions.isCompleted})`, // All sessions completed
          totalSessions: sql<number>`COUNT(DISTINCT ${participantSessions.id})`,
          completedSessions: sql<
            number
          >`COUNT(CASE WHEN ${participantSessions.isCompleted} = true THEN 1 END)`,
          ...
          totalQuestionsAnswered: sql<
            number
          >`MAX(${participantSessions.totalQuestionsAnswered})`,
        })
        .from(participants)
        .leftJoin(
          responses,
      ...
      const formattedResults = results.map((result, index) => {
        ...
        if (
          isFullyCompleted && result.firstResponseTime &&
  ...
  getEventStats: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return {
        ...
        totalCorrectAnswers,
        overallAccuracy: totalResponses > 0
